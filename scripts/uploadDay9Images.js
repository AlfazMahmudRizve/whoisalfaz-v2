...
async function run() {
  try {
    const featured = await uploadImage('monday_recipes_featured', 'monday_recipes_featured.webp');
    const body1    = await uploadImage('monday_recipes_body1',    'monday_recipes_body1.webp');
    const body2    = await uploadImage('monday_recipes_body2',    'monday_recipes_body2.webp');

    let existingMeta = {};
    if (fs.existsSync(outputMetaPath)) {
      try { existingMeta = JSON.parse(fs.readFileSync(outputMetaPath, 'utf-8')); } catch (e) {}
    }
    existingMeta.monday_recipes_featured = featured;
    existingMeta.monday_recipes_body1    = body1;
    existingMeta.monday_recipes_body2    = body2;

    fs.writeFileSync(outputMetaPath, JSON.stringify(existingMeta, null, 2), 'utf-8');
    console.log('💾 Day 9 image metadata saved to uploaded_images.json.');
  } catch (error) {
