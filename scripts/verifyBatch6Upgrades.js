...
console.log('===========================================================================');
console.log('🔍 Rigorous Verification of Batch 6 Upgraded Articles');
console.log('===========================================================================\n');

batch6Slugs.forEach(slug => {
  const filePath = `scratch/batch6_${slug}_upgraded.json`;
  ...
  // 2. SEO Description check (100 to 160 chars)
  ...
  const paras = post.body.trim().split('\n\n').filter(p => !p.startsWith('![') && !p.startsWith('By ') && !p.startsWith('## What is'));
  const firstProse = paras[0] || '';
  console.log(`  ✅ First Prose Snippet: "${firstProse.slice(0, 100)}..."`);

  // 8. Manychat affiliate check
  if (post.body.includes("partnerlinks.io") || post.body.includes("claim-manychat-bonus")) {
    console.error(`  ❌ Contains ManyChat affiliate / bonus link!`);
    totalViolations++;
  } else {
    console.log(`  ✅ Affiliate Check: Clean`);
  }
  console.log('');
});
...
if (totalViolations === 0) {
  console.log('🎉 ALL BATCH 6 ARTICLES PASSED 100% OF QUALITY GATES!');
