...
async function run() {
  try {
    const featured = await uploadImage('cold_email_machine_featured', 'cold_email_machine_featured.webp');
    const body1    = await uploadImage('cold_email_machine_body1',    'cold_email_machine_body1.webp');
    const body2    = await uploadImage('cold_email_machine_body2',    'cold_email_machine_body2.webp');
    const social   = await uploadImage('cold_email_machine_social',   'cold_email_machine_social.webp');

    let existingMeta = {};
    if (fs.existsSync(outputMetaPath)) {
      try { existingMeta = JSON.parse(fs.readFileSync(outputMetaPath, 'utf-8')); } catch (e) {}
    }
    existingMeta.cold_email_machine_featured = featured;
    existingMeta.cold_email_machine_body1    = body1;
    existingMeta.cold_email_machine_body2    = body2;
    existingMeta.cold_email_machine_social   = social;

    fs.writeFileSync(outputMetaPath, JSON.stringify(existingMeta, null, 2), 'utf-8');
    console.log('💾 Cold Email Machine image metadata saved to uploaded_images.json.');
  } catch (error) {
